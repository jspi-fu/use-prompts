...
所有文档与记忆都采用 Markdown 格式，存放于仓库根目录的 `.cursor/rules/` 及其子目录下，仅面向 AI 代理使用，不包含任何人类向说明。

在 Cursor 中如未存在 `.cursor/rules/` 或索引文件，先在仓库根目录创建目录并初始化 `.cursor/rules/index/RULE.md`。

使用 `.cursor/rules/index/RULE.md` 作为索引文档，记录所有文档的主要内容、读取场景及全局关键记忆，例如：

```
  ## 产品文档
  `.cursor/rules/idea/RULE.md` - 用户关于整个项目的想法
  `.cursor/rules/board/RULE.md` - 白板模块的整体产品文档

  ## 前端文档
  `.cursor/rules/frontend-architecture/RULE.md` - 前端架构与技术约束，修改任何前端代码时必读
  `.cursor/rules/frontend-ui-design/RULE.md` - 设计风格与样式文档，修改界面时必读

  ## 后端文档
  `.cursor/rules/backend-architecture/RULE.md` - 后端架构与技术约束，修改任何后端代码时必读
  `.cursor/rules/backend-api/RULE.md` - 后端 API 主文档，进行任何前后端对接工作时必读

  ## 当前任务文档
  `.cursor/rules/workflow/251006-fix-audio-player.mdc` - 修复音频播放器BUG
  `.cursor/rules/workflow/201007-floweditor-edge-feature.mdc` - 为工作流编辑器增加连线功能
...
- 如缺少索引文档 `.cursor/rules/index/RULE.md`，先初始化索引文档，并补齐基础技术治理信息。
...
特殊文档类型：
- 任务文档：记录复杂任务的现状分析、方案设计、背后思考、任务阶段及TODO拆解等所有内容。存放于 `.cursor/rules/workflow/` 目录下，文件名格式为 `YYMMDD-任务简述.mdc`，其中任务简述使用短横线连接的英文单词，如 `251007-fix-audio-player.mdc`。
...
- 若当前任务可能需要了解跨模块约束、设计约定、使用规范或历史决策（例如样式基准、接口契约、复用组件规则）等技术治理信息时，应先查阅 `.cursor/rules/index/RULE.md`，再按索引定位并阅读相关文档。
...
- 检查索引文档中所有的“当前任务文档”，如该文档对应开发已全部完成，则将该文档移动至 `.cursor/rules/workflow/done/`，并从“当前任务文档”部分移除。
